...
def upload_ndjson(dataset_name:str,date_str:str,rows:list[dict[str,Any]]) -> str:
    ...
    for attempt,backoff in enumerate((0,) + RETRY_BACKOFF_SECONDS,start=1):
        ...
        try:
            s3.put_object(Bucket=BUCKET_NAME,Key=key,Body=body,ContentType="application/x-ndjson")
            return key
    ...
    raise S3LoaderError(f"failed to upload s3://{BUCKET_NAME}/{key} after {MAX_RETRIES} retries: {last_error}")
